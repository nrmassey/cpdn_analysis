...
class SouthPolarAxes(PolarAxes):
	"""Stereographic polar projection for North Pole only.  
	   Longitude, latitude coordinates"""
	name = 'southpolar'

	class SouthPolarTransform(PolarAxes.PolarTransform):
		def transform(self, tr):
			xy	 = numpy.zeros(tr.shape, numpy.float_)
			t    = tr[:, 0:1] - 3.0/2 * numpy.pi
			r    = tr[:, 1:2]
			x	 = xy[:, 0:1]
			y	 = xy[:, 1:2]
			x[:] = -r * numpy.cos(t)
		...
		def inverted(self):
			return SouthPolarAxes.InvertedSouthPolarTransform()
	...
	def _set_lim_and_transforms(self):
		...
		self.transProjection = self.SouthPolarTransform()
		self.transData = (
			Affine2D().translate(0.0, 90.0) + 
